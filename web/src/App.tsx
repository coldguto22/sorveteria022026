...
function App(){
  ...
  return <div className="site-shell">
    <header className="header"><nav className="nav container" aria-label="Navegação principal"><Brand/><div className="nav-links"><a className="active" href="#inicio">Início</a><a href="#cardapio">Cardápio</a></div><div className="nav-actions"><button className="cart-button" onClick={()=>setCartOpen(true)}>▣ <span>Carrinho</span>{cart.length>0&&<b>{cart.length}</b>}</button><button className="dark-button compact" onClick={()=>setLoginOpen(true)}>Entrar</button></div></nav></header>
    <main>
      ...
      <section className="featured-section" id="cardapio"><div className="container"><div className="section-heading"><div><span className="eyebrow lilac-label">Selecionados pela casa</span><h2 id="destaques">Destaques da temporada</h2></div><a className="outline-button small" href="#familias">Ver cardápio completo →</a></div><div className="product-grid">{flavors.filter(flavor=>['Pistache & Mel','Baunilha Bourbon','Maracujá do Litoral','Lavanda & Limão Siciliano','Matcha Cerimônia'].includes(flavor.name)).map(flavor=><ProductCard key={flavor.name} flavor={flavor} onAdd={add}/>)}</div></div></section>
