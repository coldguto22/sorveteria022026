...
function CartPage({cart,onMenu,onRemove}:{cart:Flavor[];onMenu:()=>void;onRemove:(index:number)=>void}){
  if(cart.length===0) return <main className="cart-page empty-cart"><div className="empty-cart-content"><span>🍦</span><h1>Carrinho vazio</h1><p>Adicione seus sabores favoritos.</p><button onClick={onMenu}>Ver cardápio</button></div></main>
  return <main className="cart-page"><div className="cart-page-content container"><div className="cart-title"><span className="eyebrow gold-label">Seu pedido</span><h1>Meu carrinho</h1><p>{cart.length} {cart.length===1?'item selecionado':'itens selecionados'}</p></div><div className="cart-layout"><section className="cart-products">{cart.map((item,index)=><article className="cart-product" key={`${item.name}-${index}`}><div className={`cart-product-art ${item.family}`}>{item.emoji}</div><div className="cart-product-info"><span>{item.badge}</span><h2>{item.name}</h2><p>{item.subtitle}</p><button onClick={()=>onRemove(index)}>Remover</button></div><strong>R${item.price}</strong></article>)}</section><aside className="order-summary"><h2>Resumo do pedido</h2><div><span>Subtotal</span><strong>R${cart.reduce((sum,item)=>sum+item.price,0)}</strong></div><div><span>Retirada na loja</span><strong>Grátis</strong></div><hr/><div className="summary-total"><span>Total</span><strong>R${cart.reduce((sum,item)=>sum+item.price,0)}</strong></div><button className="dark-button">Continuar pedido</button><button className="continue-shopping" onClick={onMenu}>Continuar comprando</button></aside></div></div></main>
}

function App(){
  const [cart,setCart]=useState<Flavor[]>([])
  const [loginOpen,setLoginOpen]=useState(false)
  const pageFromPath=()=>window.location.pathname.includes('carrinho')?'cart':window.location.pathname.includes('cardapio')?'menu':'home'
  const [page,setPage]=useState<'home'|'menu'|'cart'>(pageFromPath)
  ...
  const navigate=(next:'home'|'menu'|'cart')=>{window.history.pushState({},'',next==='menu'?'/cardapio':next==='cart'?'/carrinho':'/');setPage(next);window.scrollTo({top:0,behavior:'smooth'})}
  const add=(flavor:Flavor)=>setCart(items=>[...items,flavor])
  return <div className="site-shell">
    <header className="header"><nav className="nav container" aria-label="Navegação principal"><span onClick={()=>navigate('home')}><Brand/></span><div className="nav-links"><a className={page==='home'?'active':''} href="/" onClick={e=>{e.preventDefault();navigate('home')}}>Início</a><a className={page==='menu'?'active':''} href="/cardapio" onClick={e=>{e.preventDefault();navigate('menu')}}>Cardápio</a></div><div className="nav-actions"><button className={`cart-button ${page==='cart'?'cart-active':''}`} onClick={()=>navigate('cart')}>▣ <span>Carrinho</span>{cart.length>0&&<b>{cart.length}</b>}</button><button className="dark-button compact" onClick={()=>setLoginOpen(true)}>Entrar</button></div></nav></header>
    {page==='home'?<main>
    ...
    </main>:page==='menu'?<MenuPage onAdd={add}/>:<CartPage cart={cart} onMenu={()=>navigate('menu')} onRemove={index=>setCart(items=>items.filter((_,itemIndex)=>itemIndex!==index))}/>}
